...
# Position of the "Speak" keyword with a small offset to the right
speak_key = Key(startX + 25, startY + 4*h + 25, 5*w, h, "Speak")
keys.append(speak_key)  # Adding the Speak button

# Position of the "Send" keyword
send_key = Key(startX+5*w+30, startY+4*h+25, 5*w, h, "Send")
keys.append(send_key)  # Adding the Send button
...
detectionCon = int(0.8)
tracker = HandTracker(detectionCon=detectionCon)
...
# Variable to store text to be sent to Gemini
text_to_send = ""
...
while True:
            ...
            if k.isOver(mouseX, mouseY) or k.isOver(signTipX, signTipY):
                ...
                if k.isOver(clickedX, clickedY):                              
                    if k.text == 'Backspace':
                        ...
                        # Store the text to be sent to Gemini
                        text_to_send = textBox.text
                        print(text_to_send)
                        # Call function to interact with Gemini and get the response
                        gemini_result = send_to_gemini(text_to_send)
                        print(gemini_result)
                        show_result = True
                        result_timer = time.time()
                        textBox.text = ''
                    elif k.text == 'Speak':
                        speak_text(gemini_result)
    ...
    # Display Gemini result on the frame for more than 1 minute
    if show_result:
        # cv2.putText(frame, text_to_send, (startX, startY - 100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
        gemini_lines = gemini_result.split('\n')
        max_lines = int(frameHeight / (40 * 1.5))
        gemini_lines = gemini_lines[:max_lines]
        for idx, line in enumerate(gemini_lines):
            result_x = startX
            result_y = startY - 40 * (len(text_to_send.split('\n')) + idx + 1)
            cv2.putText(frame, line, (result_x, result_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,0,0), 2)
        if time.time() - result_timer > 60:  # If more than 1 minute has passed
            show_result = False
